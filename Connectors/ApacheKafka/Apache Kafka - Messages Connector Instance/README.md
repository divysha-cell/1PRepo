# Apache Kafka - Messages Connector Instance
...
If the field is not found, the system uses the default environment. |False|dummy_value|
...
|Kafka Brokers|A comma-separated list of Kafka brokers to connect to, in the format `hostname:port`.|True|dummy_value|
...
|SASL PLAIN Username|The username for SASL PLAIN authentication with Kafka brokers.|False|dummy_value|
|SASL PLAIN Password|The password for SASL PLAIN authentication with Kafka brokers.|False|*****|
|Topic|The Kafka topic from which incidents are retrieved.|True|dummy_value|
|Consumer group ID|The identifier of the consumer group used when retrieving incidents.|False|dummy_value|
|Partitions|A CSV list of partitions from which to fetch messages.|False|dummy_value|
...
A positive integer fetches from that offset number.
 To start from the beginning or end of the partition, use the values 'earliest' or 'latest'.|False|dummy_value|
...
The connector extracts placeholder values from the first Google SecOps event, and placeholders are only supported for fields that contain a string value.|False|dummy_value|
...
If you do not provide a value or use an invalid template, the connector uses the fallback value "{Connector name} - Alert".|True|dummy_value|
|Rule Generator Template|A custom rule generator.
You can use placeholders in the format [field_name], for example: Phishing - [event_mailbox].
The connector extracts placeholder values from the first Google SecOps event, and placeholders are only supported for fields that contain a string value.
If you do not provide a value or use an invalid template, the connector uses the fallback value "{Connector name} - Rule Generator".|True|dummy_value|
...
|Unique ID Field|The name of the field that provides a unique message identifier. If a value is not set, the connector generates and uses a SHA-256 hash as the message identifier.|False|dummy_value|
...
|Proxy Server Address|The address of the proxy server.|False|dummy_value|
|Proxy Username|The username for proxy authentication.|False|dummy_value|
