...
|Azure Subscription ID|Microsoft Azure Subscription ID, can be viewed in Azure Portal > Subscriptions > <Your Subscription> > Subscription ID. |True|1|
|Entra ID Directory ID|Azure Entra ID Tenant ID, can be viewed in Active Directory > App Registration > <Application you configured for your integration> > Directory (tenant) ID.|True|1|
...
|Azure Resource Group|Name of Azure Resource Group where Azure Sentinel is located.|True|1|
|Azure Sentinel Workspace Name|Name of the Azure Sentinel workspace to work with, can be viewed in Azure portal > Azure Sentinel > Azure Sentinel Workspaces.|True|1|
|Client ID|Client (Application) ID that was added for the app registration in Azure Active Directory for this integration.|True|11|
...
|Incident Creation Time Filter (days)|If specified, connector will only fetch incidents that were created within the defined number of days before now.|False||
