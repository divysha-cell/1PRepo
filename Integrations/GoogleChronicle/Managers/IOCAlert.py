...
class IOCAlert:
    def __init__(
        ...
        self.alert_type = ALERT_TYPES.get("ioc")
    ...
    def get_alerts(
        ...
        if (
            ...
            self.siemplify.LOGGER.info(
                f"Last success time is greater than {ALERT_TYPE_NAMES.get(ALERT_TYPES.get('ioc'))} "
        ...
        self.siemplify.LOGGER.info(
            f"Fetched {len(filtered_alerts)} {ALERT_TYPE_NAMES.get(ALERT_TYPES.get('ioc'))} "
            f"alerts"
        )
    ...
    def pass_filters(self, alert):
        filters = get_filters_by_alert_type(
            self.siemplify.LOGGER, self.siemplify.whitelist, ALERT_TYPES.get("ioc")
        )
    ...
    def write_ids(self, existing_ids: dict[str, int] | list[str]) -> None:
        ...
        if isinstance(existing_ids, dict):
            ...
            self.siemplify.LOGGER.info(
                f"IOC existing ids is a {type(existing_ids)}. Falling back to standard SDK list write..."
            )
            write_ids(
                self.siemplify,
                existing_ids,
