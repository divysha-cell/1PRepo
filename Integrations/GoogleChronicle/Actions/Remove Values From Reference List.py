...
class RemoveValuesFromReferenceList(Action):

    def __init__(self) -> None:
        super().__init__(consts.REMOVES_VALUES_FROM_REFERENCE_LIST_SCRIPT_NAME)
    ...
    def _extract_action_parameters(self) -> None:
        ...
        self.params.reference_list_name = extract_action_param(
            self.soar_action,
            param_name="Reference List Name",
            is_mandatory=True,
            print_value=True,
        )
    ...
    def _perform_action(self, _) -> None:
        ...
        updated_reference_list = self.api_client.remove_value_from_reference_list(
            reference_list, self.params.values
        )

        self.json_results = updated_reference_list.to_json()
        self.output_message = (
            "Successfully removed values "
            f"from the reference list {reference_list.name}"
        )


def main() -> None:
    RemoveValuesFromReferenceList().run()
