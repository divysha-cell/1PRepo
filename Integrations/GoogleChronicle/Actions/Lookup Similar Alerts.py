...
from SiemplifyUtils import convert_unixtime_to_datetime, output_handler
...
def main():
    ...
    entities = extract_action_param(
        siemplify, param_name="IOCs / Assets", is_mandatory=True, print_value=True
    )
    similarity_by = extract_action_param(
        siemplify, param_name="Similarity By", is_mandatory=False, print_value=True
    )
    ...
    rule_urls = set()
    ioc_warn_message = (
        "In order to find similar alerts for IOC, action had to change the "
        '"Similarity By" value to "Only IOC/Assets".'
    )
    ...
    try:
        ...
        product_name = siemplify._current_alert.additional_properties.get(
            "product_name"
        )
        ...
        if alert_type:
            if alert_type in [consts.RULE_ALERT_TYPE, consts.EXTERNAL_ALERT_TYPE]:
                if similarity_by in [
                    consts.SIMILARITY_BY_PRODUCT,
                    consts.SIMILARITY_BY_ASSETS,
                ]:
                    rule_id = "-"
                    alerts, time_elapsed = manager.get_rule_alerts(
                        rule_id=rule_id, start_time=start_time, end_time=end_time
                    )
                    external_alerts, external_time_elapsed = list_alerts(
                        siemplify=siemplify,
                        manager=manager,
                    ...
                    assets, products, filtered_alerts = check_matches(
                        alerts=alerts,
                        similarity=similarity_by,
                        assets=entities,
                    ...
                    for al in filtered_alerts:
                        if al.alert_main_type == consts.RULE_ALERT_TYPE:
                            rule_urls.add(
                                f"{ui_root}/ruleDetections?ruleId={al.rule_id}"
                            )
                    ...
                    if alert_type == consts.RULE_ALERT_TYPE:
                        ...
                        alerts, time_elapsed = list_alerts(
                            siemplify=siemplify,
                            manager=manager,
                            start_time=start_time,
                            end_time=end_time
                ...
                assets, products, filtered_alerts = check_matches(
                    alerts=alerts,
                    similarity=consts.SIMILARITY_BY_ASSETS,
                    assets=entities,
                    alert_name=alert_name,
                    product_name=product_name,
                    alert_type=alert_type,
            ...
            if filtered_alerts:
                output_message = (
                    "Successfully counted similar alerts from the provided time"
                    " frame in Google Chronicle"
                )
                if (
                    alert_type == consts.IOC_ALERT_TYPE
                    and similarity_by != consts.SIMILARITY_BY_ASSETS
                ):
                    output_message += f"\n\n{ioc_warn_message}"
                    similarity_by = consts.SIMILARITY_BY_ASSETS

                json_results, table_results = create_results(
                    similarity=similarity_by,
                    assets=assets,
                    products=products,
                    ...
                    alert_name=alert_name,
                    alert_type=alert_type,
                    rule_urls=list(rule_urls),
                )

                result_json = {
                    "count": len(filtered_alerts),
                    "distinct": json_results,
                    "processed_alerts": total_processed_events,
                ...
                if not external_url:
                    ...
                    external_url = utils.construct_url(
                        consts.LOOKUP_SIMILAR_ALERTS_URI,
                ...
                output_message = (
                    "No similar alerts were found from the provided time frame"
                    " in Google Chronicle"
                )
            ...
            result_value = False
            output_message = (
                "Action wasn't able to retrieve data about similar alerts."
                " Make sure that this action is executed on the alert from"
                ' "Chronicle Alerts Connector".'
            )

    except exceptions.GoogleChronicleAPILimitError as e:
        output_message = (
            f'Error executing action "{SCRIPT_NAME}". Reason: all of the'
...
def list_alerts(
        siemplify: SiemplifyAction,
        manager: GoogleChronicleManager | GoogleChronicleManagerV2,
        start_time: str,
        end_time: str
) -> ([datamodels.Alert], float):
    """
    List alerts

    Args:
        siemplify (SiemplifyAction): SiemplifyAction object
        manager (GoogleChronicleManager | GoogleChronicleManagerV2): manager object
        start_time (str): start time for the time range to query alerts
        end_time (str): end time for the time range to query alerts

    Returns:
        ([datamodels.Alert], float): list of Alert objects, and elapsed time
    """
    try:
        alerts, time_elapsed = manager.list_alerts(
            start_time=start_time,
            end_time=end_time,
            limit=consts.LIST_ALERTS_LIMIT,
            fetch_user_alerts=True,
        )
    except exceptions.GoogleChronicleManagerError as e:
        siemplify.LOGGER.info(f"Failed to get alerts. Error: {e}.")
        return [], 0

    return alerts, time_elapsed
...
def check_matches(alerts, similarity, assets, alert_name, product_name, alert_type):
    ...
    for alert in alerts:
        if similarity == consts.SIMILARITY_BY_NAME_AND_PRODUCT:
            matching_products = [
                name
                for name in utils.convert_comma_separated_to_list(product_name)
                if name in alert.get_product_names_list
            ]
            is_same_name = (
                alert.name.lower() == alert_name.lower()
                if alert_type == consts.EXTERNAL_ALERT_TYPE
                else True
            )
...
def create_results(
    similarity, assets, products, filtered_alerts, alert_name, alert_type, rule_urls
):
    json_results = []
    table_results = {}
    ...
    if similarity == consts.SIMILARITY_BY_NAME_AND_PRODUCT:
        for asset in assets:
            ...
            matched_alerts = [
                al for al in matched_alerts if asset.lower() in al.get_all_entities
            ]
            if matched_alerts:
                ...
                matched_products = list(
                    {
                        alert.get_unique_product_name
                        for alert in matched_alerts
                        if alert.get_unique_product_name
                    }
                )
                for product in matched_products:
                    product_alerts = [
                        ...
                        if product == al.get_unique_product_name
                    ]
                    if product_alerts:
                        json_results.append(
                            to_json(
            ...
            if matched_alerts:
                ...
                for product in matched_products:
                    ...
                    if product_alerts:
                        ...
                        for name in names:
                            name_alerts = [
                                al for al in product_alerts if name == al.name
                            ]
                            if name_alerts:
                                json_results.append(
                                    to_json(
...
def to_json(alerts, product_name, entity, alert_name, alert_type, rule_urls):
    ...
    json_data = {
        ...
        "email_addresses": (
            utils.convert_list_to_comma_string(list(set(emails))) or ""
        ),
...
def to_table(alerts, product_name, alert_name, alert_type):
    ...
    return {
        ...
        "Hostnames": (
            utils.convert_list_to_comma_string(list(set(hostnames))) or "N/A"
        ),
        ...
        "Email Addresses": (
            utils.convert_list_to_comma_string(list(set(emails))) or "N/A"
        ),
        "Hashes": (utils.convert_list_to_comma_string(list(set(hashes))) or "N/A"),
        "Processes": (
            utils.convert_list_to_comma_string(list(set(processes))) or "N/A"
        ),
    }
