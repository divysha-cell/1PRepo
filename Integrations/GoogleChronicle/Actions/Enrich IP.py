    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                siemplify.LOGGER.info(
                    f"Fetching information for address {entity.identifier}"
                )
                ...
                entity.additional_properties.update(
                    ioc_details.as_enrichment(prefix=ENRICHMENT_PREFIX)
                )
                entity.is_enriched = True

                json_results[entity.identifier] = ioc_details.to_json()

                if ioc_details.highest_source_severity[
                    1
                ] >= lowest_suspicious_severity or (
                ...
                siemplify.result.add_entity_table(
                    entity.identifier, construct_csv(ioc_details.to_table())
                )
        ...
        if successful_entities:
            output_message += (
                "Successfully enriched the following IPs from Google"
                " Chronicle:\n   {}".format(
        ...
        if failed_entities:
            output_message += (
                "\n\nAction was not able to enrich the following IPs from"
                " Google Chronicle:\n   {}".format(
