...
from constants import REMOVE_FROM_WHITELIST_SCRIPT_NAME
...
class RemoveFromWhitelistAction(ZscalerBaseAction):
    def __init__(self) -> None:
        super().__init__(REMOVE_FROM_WHITELIST_SCRIPT_NAME)
    ...
    def _execute_bulk_operation(self) -> bool:
        ...
        try:
            self.api_client.remove_multiple_from_whitelist(list(self._items_to_process))
            return True
        except ZscalerManagerError as e:
            self.logger.error(f"Failed to remove entities from whitelist: {str(e)}")
            self.output_message = (
                f"Failed to remove entities from whitelist.\n{str(e)}."
            )
    ...
    def _construct_output_message(self) -> None:
        ...
        if not self.successful_entities and not self.failed_entities:
            self.output_message = "No entities were removed from the Urls whitelist"
            self.result_value = False


def main() -> None:
    RemoveFromWhitelistAction().run()
