...
from constants import ADD_TO_WHITELIST_SCRIPT_NAME
...
class AddToWhitelistAction(ZscalerBaseAction):
    def __init__(self) -> None:
        super().__init__(ADD_TO_WHITELIST_SCRIPT_NAME)
        self.successful_entities: list[str] = []
        self.already_exists_entities: list[str] = []
    ...
    def _perform_action(self, entity: Entity) -> None:
        """Perform the action."""
        entity_to_allow = self._get_entity_to_allow(entity)

        if not entity_to_allow:
            return

        self._items_to_process.add(entity_to_allow)
        self.successful_entities.append(entity.identifier)

    def _get_entity_to_allow(self, entity: Entity) -> str | None:
        """Extract entity value to allow."""
    ...
    def _execute_bulk_operation(self) -> bool:
        ...
        try:
            self.api_client.add_multiple_to_whitelist(list(self._items_to_process))
            return True
        except ZscalerManagerError as e:
            self.logger.error(f"Failed to add entities to whitelist: {str(e)}")
            self.output_message = f"Failed to add entities to whitelist.\n{str(e)}."
    ...
    def _construct_output_message(self) -> None:
        ...
        if not self.successful_entities and not self.failed_entities:
            self.output_message = "No entities were added to the Urls whitelist"
            self.result_value = False


def main() -> None:
    AddToWhitelistAction().run()
