...
from constants import UNISOLATE_ENDPOINT_ACTION_SCRIPT_NAME, AGENT_ID_VALIDATION_REGEX
...
def main():
    ...
    siemplify.script_name = UNISOLATE_ENDPOINT_ACTION_SCRIPT_NAME
    ...
    if not siemplify.target_entities and not agent_ids:
        output_message = "No entities or agent IDs were provided to un-isolate."
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                siemplify.LOGGER.info(
                    f"Initiating unisolation of endpoint {entity.identifier}"
                )
        ...
        for agent_id in agent_ids:
            ...
            result: str = process_single_agent_unisolation(
                siemplify,
                xdr_manager,
                agent_id,
        ...
        if all_successful:
            endpoints_str = "\n   ".join(all_successful)
            output_message += (
                "Successfully initiated unisolation of the following endpoints:\n"
                f"   {endpoints_str}"
            )
            if successful_entities:
                siemplify.update_entities(successful_entities)
        else:
            output_message += "No endpoints were unisolated."
        ...
        if all_failed:
            failed_str: str = "\n   ".join(all_failed)
            output_message += (
                f"\n\nFailed to un-isolate the following endpoints:\n   {failed_str}"
            )
...
def process_single_agent_unisolation(
    siemplify: SiemplifyAction,
    xdr_manager: XDRManager,
    agent_id: str,
    processed_endpoint_ids: set[str],
) -> str:
    """Processes a single agent ID for un-isolation.
    ...
    try:
        ...
        siemplify.LOGGER.info(
            f"Initiating unisolation for Agent ID: {agent_id}"
        )
        xdr_manager.unisolate_endpoint(agent_id)
        siemplify.LOGGER.info(
            f"Successfully initiated un-isolation for Agent ID: {agent_id}"
        )
        return "success"
